...
# خواندن داده خام
eeg_data = np.load(RAW_EEG_PATH)    # (n_subject, n_channels, n_samples)
labels = np.load(RAW_LABELS_PATH)   # (n_subject,)

# اگر نیاز به استخراج برچسب از فایل excel هست، این قسمت را فعال کنید:
# subject_info = pd.read_excel(SUBJECT_INFO_PATH)
# labels = subject_info['Diagnosis'].apply(lambda x: 1 if x == 'MDD' else 0).values
